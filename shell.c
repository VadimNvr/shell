...
char ***massiv_prog(char **line, char *separator)
{
    ...
    *str = NULL; //а вот и он :) NULL у нас всегда будет последним элементом str

    //это аргументы текущей программы
    char **tmpprog = (char **) malloc(sizeof(char *)); //сначала там будет единственный элемент NULL
    *tmpprog = NULL; //неожиданно, да?) NULL у нас всегда будет последним элементом tmpprog
...
int run(char **prog, int *lpipe, int *rpipe)
{
    ...
    switch (fork())
    {
        ...
        case  0:
            ...
            //удаляем лищние аргументы перенаправления из задачи
